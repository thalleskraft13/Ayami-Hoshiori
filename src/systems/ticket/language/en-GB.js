...
module.exports = [
  { id: "home_title", render: (ctx) =>
    `# 🎫 Ticket System ${ctx.animada}\nHiya! I'm Ayami ${ctx.corao} and I'll help you set up support on your server!` },
  ...
  { id: "publish_no_channel", render: (ctx) => `${ctx.emduvida} Pick a channel first, yeah?` },
  ...
  { id: "ask_ticket_name", render: (ctx) => `${ctx.pensando} What shall the ticket be called?\nUse \`{count}\` for the number. E.g.: \`ticket-{count}\`` },
  ...
  { id: "ask_modal_title", render: (ctx) => `${ctx.pensando} What's the modal's title, then?` },
  ...
  { id: "ask_option_ticket_name", render: (ctx) => `${ctx.pensando} What shall tickets created by this option be named? Use \`{count}\` for the number.` },
  ...
  { id: "default_seq_intro_desc", render: () =>
    "Hello {user}! I'll ask you a few questions.\nYou have {timeout}s to answer each one." },
  ...
  { id: "eb_color_select_placeholder", render: () => "🎨 Choose colour…" },
  ...
  { id: "eb_custom_hex_modal_title", render: () => "Custom HEX Colour" },
  ...
  { id: "claim_status_none", render: () => "👤 **Assigned to:** No one yet" },
  { id: "claim_status_set", render: (ctx) => `👤 **Assigned to:** <@${ctx.userId}>` },
  { id: "btn_claim", render: () => "🖐️ Claim Ticket" },
  { id: "btn_unclaim", render: () => "🔓 Release Ticket" },
  { id: "btn_transfer", render: () => "🔄 Transfer" },
  { id: "claim_no_permission", render: () => "❌ You don't have permission to claim this ticket." },
  { id: "claim_already_claimed", render: (ctx) => `${ctx.emburrada} This ticket has already been claimed by <@${ctx.userId}>.` },
  { id: "claim_success", render: (ctx) => `${ctx.feliz} <@${ctx.userId}> claimed this ticket.` },
  { id: "unclaim_no_permission", render: () => "❌ You don't have permission to release this ticket." },
  { id: "unclaim_not_claimed", render: () => "This ticket doesn't have an assignee yet." },
  { id: "unclaim_success", render: (ctx) => `🔓 <@${ctx.userId}> released this ticket.` },
  { id: "transfer_no_permission", render: () => "❌ You don't have permission to transfer this ticket." },
  { id: "transfer_select_placeholder", render: () => "Select the new assignee" },
  { id: "transfer_same_user", render: () => "That user is already the assignee for this ticket." },
  { id: "transfer_success", render: (ctx) => `🔄 <@${ctx.fromUserId}> transferred the ticket to <@${ctx.toUserId}>.` },
  { id: "log_claim_title", render: () => "🖐️ Ticket claimed" },
  { id: "log_unclaim_title", render: () => "🔓 Ticket released" },
  { id: "log_transfer_title", render: () => "🔄 Assignee transferred" },
  { id: "log_field_channel", render: () => "Channel" },
  { id: "log_field_by", render: () => "Performed by" },
  { id: "log_field_previous", render: () => "Previous assignee" },
  { id: "log_field_new", render: () => "New assignee" },
  { id: "log_none", render: () => "No one" },

  { id: "participant_ticket_not_found", render: () => "❌ Could not find data for this ticket." },
  { id: "participant_add_no_permission", render: () => "❌ You don't have permission to add participants." },
  { id: "participant_remove_no_permission", render: () => "❌ You don't have permission to remove participants." },
  { id: "participant_add_placeholder", render: () => "Select the member to add" },
  { id: "participant_remove_placeholder", render: () => "Select the member to remove" },
  { id: "participant_cannot_add_bot", render: () => "❌ You can't add the bot as a participant." },
  { id: "participant_already_in", render: (ctx) => `<@${ctx.userId}> is already part of this ticket.` },
  { id: "participant_add_success", render: (ctx) => `➕ <@${ctx.userId}> was added to the ticket.` },
  { id: "participant_cannot_remove_owner", render: () => "❌ You can't remove the ticket creator." },
  { id: "participant_cannot_remove_bot", render: () => "❌ You can't remove the bot from the ticket." },
  { id: "participant_remove_success", render: (ctx) => `➖ <@${ctx.userId}> was removed from the ticket.` },
  { id: "log_participant_add_title", render: () => "➕ Participant added" },
  { id: "log_participant_remove_title", render: () => "➖ Participant removed" },
  { id: "log_field_participant", render: () => "Participant" },
  { id: "btn_add_participant", render: () => "➕ Add Participant" },
  { id: "btn_remove_participant", render: () => "➖ Remove Participant" },
  { id: "btn_close", render: () => "🔒 Close Ticket" },
  { id: "action_menu_title", render: () => "🎫 Ticket Actions" },

  { id: "faq_select_placeholder", render: () => "Select a question" },
  { id: "faq_open_ticket_button", render: () => "🎫 Open Ticket" },
  { id: "faq_close_button", render: () => "❌ Close" },
  { id: "faq_closed_message", render: () => "Okay! Feel free to open a ticket again if you need to." },
  { id: "faq_list_title", render: () => "❓ Frequently Asked Questions" },
  { id: "faq_list_desc", render: () => "Your question might already be answered here. Pick a question below or open a ticket directly." },

  { id: "opt_template", render: () => "Template Library" },
  { id: "opt_template_desc", render: () => "Publish or install ticket templates" },
  { id: "btn_template_publish", render: () => "📦 Publish as Template" },
  { id: "btn_template_install", render: () => "📥 Install Template" },
  { id: "modal_template_publish_title", render: () => "Publish Template" },
  { id: "modal_template_name_label", render: () => "Template name" },
  { id: "modal_template_short_desc_label", render: () => "Short description" },
  { id: "modal_template_tags_label", render: () => "Tags" },
  { id: "modal_template_tags_placeholder", render: () => "support, sales, community" },
  { id: "template_publish_success", render: (ctx) => `✅ Template published! ID: \`${ctx.libId}\`` },
  { id: "template_publish_error", render: (ctx) => `❌ Error publishing template: ${ctx.error}` },
  { id: "modal_template_install_title", render: () => "Install Template" },
  { id: "modal_template_libid_label", render: () => "Template ID" },
  { id: "template_install_success", render: (ctx) => `✅ Template installed as new panel: \`${ctx.panelId}\`` },
  { id: "template_install_error", render: (ctx) => `❌ Error installing template: ${ctx.error}` },
  { id: "template_menu_header", render: () => "## 📦 Template Library\nPublish this panel as a reusable template or install an existing template as a new panel." },

  { id: "opt_claim", render: () => "Assignee (Claim)" },
  { id: "opt_claim_desc", render: () => "Configure who can claim tickets" },
  { id: "opt_participants", render: () => "Participants" },
  { id: "opt_participants_desc", render: () => "Configure who can add/remove participants" },
  { id: "opt_faq", render: () => "FAQ" },
  { id: "opt_faq_desc", render: () => "Frequently asked questions before opening a ticket" },
  { id: "opt_actionmenu", render: () => "Action Menu" },
  { id: "opt_actionmenu_desc", render: () => "Keywords that open the action menu" },

  { id: "btn_clear_roles", render: () => "🧹 Clear roles" },
  { id: "claim_roles_placeholder", render: () => "Roles that can claim tickets" },
  { id: "claim_log_channel_placeholder", render: () => "Claim log channel (optional)" },
  { id: "claim_roles_none", render: () => "All staff members" },
  { id: "claim_menu_header", render: (ctx) => `## 👤 Ticket Assignee (Claim)\n**Status:** ${ctx.status}\n**Allowed roles:** ${ctx.roles}\n**Log channel:** ${ctx.logChannel}` },

  { id: "participants_add_roles_placeholder", render: () => "Roles that can add participants" },
  { id: "participants_remove_roles_placeholder", render: () => "Roles that can remove participants" },
  { id: "participants_menu_header", render: (ctx) => `## 👥 Participants\n**Can add:** ${ctx.addRoles}\n**Can remove:** ${ctx.removeRoles}` },

  { id: "faq_add_button", render: () => "➕ Add Question" },
  { id: "faq_limit_reached", render: (ctx) => `❌ Limit of ${ctx.max} questions reached for your plan.` },
  { id: "modal_faq_add_title", render: () => "New FAQ Entry" },
  { id: "modal_faq_question_label", render: () => "Question" },
  { id: "modal_faq_answer_label", render: () => "Answer" },
  { id: "faq_added_success", render: () => "✅ Question added!" },
  { id: "faq_remove_placeholder", render: () => "Select a question to remove" },
  { id: "faq_menu_header", render: (ctx) => `## ❓ Pre-Opening FAQ\n**Status:** ${ctx.status}\n**Questions:** ${ctx.count}/${ctx.max}` },

  { id: "actionmenu_add_button", render: () => "➕ New Keyword" },
  { id: "actionmenu_premium_required", render: () => "❌ Multiple keywords are exclusive to the Lua Crescente plan." },
  { id: "modal_actionmenu_add_title", render: () => "New Keyword" },
  { id: "modal_actionmenu_keyword_label", render: () => "Exact keyword" },
  { id: "actionmenu_added_success", render: () => "✅ Keyword added!" },
  { id: "actionmenu_edit_placeholder", render: () => "Select a keyword to edit" },
  { id: "actionmenu_menu_header", render: (ctx) => `## 🎫 Action Menu\n**Status:** ${ctx.status}\n**Keywords:** ${ctx.count}\n**Plan:** ${ctx.plan}` },
  { id: "actionmenu_plan_premium", render: () => "Lua Crescente (multiple keywords)" },
  { id: "actionmenu_plan_free", render: () => "Free (1 fixed keyword)" },
  { id: "actionmenu_actions_placeholder", render: () => "Actions available in this menu" },
  { id: "actionmenu_roles_placeholder", render: () => "Roles that can use this keyword" },
  { id: "actionmenu_autodelete_on", render: () => "🗑️ Delete message: Enable" },
  { id: "actionmenu_autodelete_off", render: () => "🗑️ Delete message: Disable" },
  { id: "actionmenu_delete_keyword", render: () => "❌ Delete Keyword" },
  { id: "actionmenu_keyword_header", render: (ctx) => `## 🔑 Keyword: \`${ctx.keyword}\`\n**Actions:** ${ctx.actions}\n**Allowed roles:** ${ctx.roles}\n**Auto-delete message:** ${ctx.autoDelete}` },
];
